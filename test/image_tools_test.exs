defmodule ImageToolsTest do
  ...
  test "create_thumbnail/4 handles invalid input" do
    ...
    assert {:error, "quality must be >= 0"} = ImageTools.create_thumbnail(@test_image, 320, 240, quality: -1)
    assert {:error, "quality must be <= 100"} = ImageTools.create_thumbnail(@test_image, 320, 240, quality: 101)
    assert {:error, "target_size must be > 0"} = ImageTools.create_thumbnail(@test_image, 320, 240, target_size: 0)
  end
end
